...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      await submit(form);
      // Navigation happens inside useCheckout → /order-success/:id
    } catch {
          ...
          <div className="lg:col-span-2 bg-white rounded-lg shadow-sm p-6 sm:p-8 space-y-6">
            ...
            {step === 1 && (
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <input className={inputClass} name="first_name" placeholder="First Name" value={form.first_name} onChange={handleChange} />
                  <input className={inputClass} name="last_name" placeholder="Last Name" value={form.last_name} onChange={handleChange} />
                </div>

                <input className={inputClass} name="email" placeholder="Email Address" type="email" value={form.email} onChange={handleChange} />

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <input className={inputClass} name="city" placeholder="City" value={form.city} onChange={handleChange} />
                  <input className={inputClass} name="postal_code" placeholder="Postal Code" value={form.postal_code} onChange={handleChange} />
                  <select className={inputClass} name="country" value={form.country} onChange={handleChange}>
                ...
                <input className={inputClass} name="phone" placeholder="Phone Number" type="tel" value={form.phone} onChange={handleChange} />
            ...
            {step === 2 && (
              <>
                ...
                {/* Stripe-ready placeholder — will be replaced by Stripe Elements */}
            ...
            {step === 3 && (
              <>
                ...
                <div className="bg-gray-50 rounded-sm p-4 text-sm space-y-1">
                  ...
                  <p className="text-gray-500">{form.city}{form.postal_code ? `, ${form.postal_code}` : ""}{form.country ? `, ${form.country}` : ""}</p>
                ...
                {cart.length === 0 && (
                  <p className="text-yellow-600 text-xs">Your cart is empty. <Link to="/shop" className="underline">Continue shopping</Link></p>
                )}
